...
const PRESETS: Preset[] = [
  ...
  {
    name: "Short Sprints",
    description: "15m focus / 3m break",
    settings: {
      focusDuration: 15,
      shortBreak: 3,
      longBreak: 10,
  ...
  {
    name: "Gentle Start",
    description: "10m focus / 5m break",
    settings: {
      focusDuration: 10,
      shortBreak: 5,
      longBreak: 10,
...
export default function SettingsPage() {
  ...
  function handleSoundChange(sound: NotificationSound) {
    dispatch({
      ...
      payload: { notificationSound: sound },
    });
    toast.success(
      `Notification sound: ${NOTIFICATION_SOUNDS.find(s => s.id === sound)?.label || sound}`
    );
  }
  ...
  const handleSwitchToSheets = useCallback(
    async (checked: boolean) => {
      if (checked) {
        setMode("sheets");
        toast.info(
          "Switched to Google Sheets mode. Configure your Sheet below."
        );
      } else {
        setMode("file");
      ...
      try {
        ...
        const res = await fetch("/api/trpc/data.importData", {
          ...
          body: JSON.stringify({
            json: {
              content: text,
              format: file.name.endsWith(".json") ? "json" : "md",
            },
          }),
          credentials: "include",
        });
    ...
    <div className="p-4 lg:p-8 max-w-3xl">
      ...
      <div className="bg-card rounded-2xl border border-border p-6 mb-6">
        ...
        <p className="text-xs text-muted-foreground mb-5">
          Choose where your data lives. Local Markdown file is the default —
          your data is stored on the server in a readable{" "}
          <code className="bg-warm-sand/60 px-1 py-0.5 rounded">.md</code> file.
        </p>
        ...
        >
          <div className="flex items-center gap-2 mb-2">
            ...
            {(mode === "file" || mode === "local") && (
              <span className="ml-auto text-xs bg-warm-sage/10 text-warm-sage px-2 py-0.5 rounded-full font-medium">
                Active
              </span>
            )}
          </div>
          <p className="text-xs text-muted-foreground mb-3">
            Data is saved as a human-readable{" "}
            <code className="bg-warm-sand/60 px-1 py-0.5 rounded">
              focus-assist-data.md
            </code>{" "}
            file. Auto-saves on every change.
          </p>
          {(mode === "file" || mode === "local") && (
          ...
          <div className="flex items-center gap-2 mb-2">
            ...
            <div className="ml-auto flex items-center gap-2">
              {mode === "sheets" && sheetsConfigured && (
                <span className="text-xs bg-warm-blue/10 text-warm-blue px-2 py-0.5 rounded-full font-medium">
                  Active
                </span>
              )}
              <Switch
            ...
            (sheetsConfigured ? (
              <div>
                <div className="bg-warm-blue-light/50 rounded-lg p-3 mb-3 flex items-center gap-2">
                  <Cloud className="w-4 h-4 text-warm-blue" />
                  <span className="text-xs text-warm-blue font-medium">
                ...
                <Button
                  ...
                  variant="outline"
                  size="sm"
                  className="text-warm-terracotta border-warm-terracotta/30 hover:bg-warm-terracotta-light gap-1.5 text-xs"
                >
              ...
              <div className="space-y-3">
                ...
                <Button
                  ...
                  className="bg-warm-blue hover:bg-warm-blue/90 text-white gap-2 w-full text-xs"
                  disabled={!sheetId || !apiKey}
                >
      ...
      <div className="bg-card rounded-2xl border border-border p-6 mb-6">
        ...
        <p className="text-xs text-muted-foreground mb-4">
          Export your tasks and focus data to an Obsidian vault. Set the vault
          path and enable auto-sync to keep your notes updated. The server will
          write a{" "}
          <code className="bg-warm-sand/60 px-1 py-0.5 rounded">
            FocusAssist.md
          </code>{" "}
          ...
          <div>
            ...
            <div className="flex gap-2">
              ...
              <Button
                ...
                size="sm"
                className="bg-warm-lavender hover:bg-warm-lavender/90 text-white gap-1.5 shrink-0"
              >
                <Save className="w-3.5 h-3.5" /> Save
              </Button>
            </div>
            {obsidianPath && (
              <p className="text-xs text-warm-lavender mt-1.5">
                Current: {obsidianPath}
              </p>
            )}
          </div>
          <div className="flex items-center justify-between bg-background rounded-xl border border-border p-4">
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Automatically export on every data change
              </p>
            </div>
            <Switch
      ...
      <div className="bg-card rounded-2xl border border-border p-6 mb-6">
        ...
        <p className="text-xs text-muted-foreground mb-4">
          Download your data as a Markdown or JSON file for backup, or import a
          previously exported file to restore.
        </p>
      ...
      <div className="bg-card rounded-2xl border border-border p-6 mb-6">
        ...
        <p className="text-xs text-muted-foreground mb-4">
          Scan your data for inconsistencies (orphaned references, missing
          fields, duplicate IDs) and auto-fix what can be repaired.
        </p>
        <Button
        ...
        {integrityResult && (
          ...
          >
            ...
            {integrityResult.fixed.length > 0 && (
              <div className="mt-2">
                <p className="text-xs font-medium text-warm-sage">
                  Auto-fixed:
                </p>
      ...
      <div className="bg-warm-sand/50 rounded-2xl border border-border p-6">
        ...
        <p className="text-xs text-muted-foreground leading-relaxed mb-4">
          Built with ADHD-friendly design principles in mind. This app uses
          color-coding, chunky interactive elements, and satisfying feedback to
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <span className="text-xs px-3 py-1 rounded-full bg-warm-blue-light border border-warm-blue/20 text-warm-blue font-medium">
            Google Sheets Sync
          </span>
          <span className="text-xs px-3 py-1 rounded-full bg-warm-lavender/10 border border-warm-lavender/20 text-warm-lavender font-medium">
            Obsidian Sync
          </span>
        </div>
      </div>
