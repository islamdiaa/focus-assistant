...
| **Canvas**            | Freeform thought journal with rich text editing powered by Tiptap. Daily entries with date navigation, markdown shortcuts (`# `, `- `, `[ ] `, `> `), slash command menu (`/`), floating formatting toolbar, task/checkbox lists, callout blocks, and auto-save (1s debounce + 30s interval + save on blur). Designed as an ADHD-friendly "brain dump" space with time-aware placeholders and zero-friction start. |
...
- **Test job:** TypeScript type-checking + Vitest (324+ tests)
...
- **Tests:** Vitest (324+ tests including schema integrity + persistence round-trips)
