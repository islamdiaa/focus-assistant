...
function statsToRows(stats: DailyStats[]): string[][] {
  ...
  return [
    ...
    ...stats.map(s => [
      s.date,
      String(s.tasksCompleted),
      String(s.focusMinutes),
      String(s.pomodorosCompleted),
    ]),
  ];
}
...
function rowsToSettings(rows: string[][]): {
  ...
  try {
    const results = await Promise.all([
      ...
      writeSheet(
        ...
        apiKey,
        "Pomodoros",
        pomodorosToRows(state.pomodoros)
      ),
      writeSheet(
        sheetId,
        apiKey,
        "Settings",
        settingsToRows(state.settings, state.currentStreak)
      ),
