...
export function generateObsidianMarkdown(state: AppState): string {
  ...
  lines.push('');
  lines.push(`> Last synced: ${now.toLocaleString('en-US', { dateStyle: 'medium', timeStyle: 'short' })}`);
 ...
 */
export async function syncToObsidian(state: AppState): Promise<{ synced: boolean; error?: string }> {
