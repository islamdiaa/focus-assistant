...
if (existsSync(settingsPath)) {
  ...
  if (
    settingsContent.includes("pkgJson.version") ||
    settingsContent.includes("package.json")
  ) {
    ...
    if (vMatch) {
      if (vMatch[1] === pkgVersion) {
        ...
        fail(
          `SettingsPage version mismatch: v${vMatch[1]} (expected v${pkgVersion})`
        );
      }
    } else {
...
if (existsSync(dockerfilePath)) {
  ...
  const dMatch = dockerContent.match(
  ...
  if (dMatch) {
    if (dMatch[1] === pkgVersion) {
      ...
      fail(
        `Dockerfile version label mismatch: ${dMatch[1]} (expected ${pkgVersion})`
      );
    }
  } else {
...
if (existsSync(changelogPath)) {
  ...
  if (changelog.includes(`[${pkgVersion}]`)) {
    ...
    fail(
      `CHANGELOG.md missing section for [${pkgVersion}] — update before pushing`
    );
  }
}
...
if (existsSync(homePath)) {
  ...
  while ((match = shortcutPattern.exec(homeContent)) !== null) {
    ...
    if (
      !context.includes("!isMod") &&
      !context.includes("!e.metaKey") &&
      !context.includes("!e.ctrlKey")
    ) {
      // Check the same line
      ...
      if (
        !line.includes("!isMod") &&
        !line.includes("!e.metaKey") &&
        !line.includes("!e.ctrlKey")
      ) {
  ...
  if (badShortcuts.length === 0) {
    ...
    badShortcuts.forEach(k =>
      fail(
        `Shortcut '${k}' missing modifier key check (!isMod) — will intercept Cmd+${k.toUpperCase()}`
      )
    );
  }
}
...
if (existsSync(mdStoragePath)) {
  ...
  if (
    mdContent.includes("col(") ||
    mdContent.includes("col =") ||
    mdContent.includes("safeGet")
  ) {
    ...
    if (hardcodedPattern.test(mdContent)) {
      warn(
        "mdStorage uses hardcoded high indices — consider using a safe column accessor"
      );
    } else {
...
if (existsSync(sheetsPath)) {
  ...
  if (
    sheetsContent.includes("retry") ||
    sheetsContent.includes("Retry") ||
    sheetsContent.includes("MAX_RETRIES")
  ) {
    pass("Save logic includes retry mechanism");
  } else {
    warn(
      "Save logic has no retry mechanism — save failures may cause data loss"
    );
  }
  if (
    sheetsContent.includes("onSaveError") ||
    sheetsContent.includes("saveError") ||
    sheetsContent.includes("onError")
  ) {
...
if (failures === 0 && warnings === 0) {
  ...
  console.log(
    `  🟡 ${warnings} warning(s), 0 failures. Review warnings before pushing.\n`
  );
  process.exit(0);
} else {
  console.log(
    `  🔴 ${failures} failure(s), ${warnings} warning(s). Fix failures before pushing.\n`
  );
  process.exit(1);
}
